...
const MOCK_ORDERS = [
  {
    ...
    totalAmount: 2500000,
    orderDetails: [
      { orderDetailId: 1, productName: "Vợt cầu lông Yonex Astrox 88D Pro", quantity: 1, unitPrice: 2500000, isStringingService: false },
    ],
  },
  {
    orderId: 1002,
    orderDate: "2026-06-01T14:30:00",
    status: "Chờ xác nhận",
    receiverName: "Nguyễn Văn An",
    phoneNumber: "0901234567",
    shippingAddress: "123 Nguyễn Trãi, Quận 1, TP.HCM",
    ...
    totalAmount: 880000,
    orderDetails: [
      { orderDetailId: 2, productName: "Giày cầu lông Victor A362III", quantity: 1, unitPrice: 850000, isStringingService: false },
    ],
  },
  {
    orderId: 1003,
    orderDate: "2026-06-05T09:00:00",
    ...
    totalAmount: 1500000,
    orderDetails: [
      { orderDetailId: 3, productName: "Balo cầu lông Yonex 92426EX", quantity: 1, unitPrice: 1200000, isStringingService: false },
      { orderDetailId: 4, productName: "Quả cầu lông Yonex AS-50", quantity: 3, unitPrice: 100000, isStringingService: false },
    ],
  },
];
...
const getStatusConfig = (s) =>
  STATUS_CONFIG[s] || { color: "text-gray-500", bg: "bg-gray-50", border: "border-gray-200", dot: "bg-gray-400", icon: "•" };
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border ${c.bg} ${c.color} ${c.border}`}>
...
const OrderDetailPanel = ({ order, onClose }) => {
  const c = getStatusConfig(order.status);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:max-w-2xl max-h-[92vh] sm:max-h-[85vh] bg-white sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
        ...
        <div className={`flex items-start justify-between p-5 border-b border-gray-100 ${c.bg}`}>
          <div>
        ...
        {canCancel && (
          <div className="p-4 border-t border-gray-100 bg-white">
            <button
              ...
              disabled={cancelling}
              className="w-full py-2.5 rounded-xl bg-red-50 text-red-600 border border-red-200 font-semibold text-sm hover:bg-red-100 active:scale-[0.98] transition-all disabled:opacity-50"
            >
              Hủy đơn hàng
            </button>
          </div>
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0">
          ...
          <p className="text-sm font-semibold text-gray-800 truncate mt-0.5">
            {firstProduct}
            {itemCount > 1 && <span className="text-gray-400 font-normal"> +{itemCount - 1} sản phẩm</span>}
          </p>
        </div>
        <StatusBadge status={order.status} />
      </div>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 text-xs text-gray-400">
          <span>🕐</span><span>{formatDate(order.orderDate)}</span>
        </div>
