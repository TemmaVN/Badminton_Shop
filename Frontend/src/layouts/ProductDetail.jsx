...
const addToCart = async (_detailId, _qty) => {
...
useEffect(() => {
  const loadProduct = async () => {
    ...
    try {
      const result = await getProductDetailBySlug(productSlug);
      if (result) {
        setProduct(result);
      }
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-baseline gap-4 mb-6">
              <span className="text-4xl font-bold text-orange-500">{formatPrice(selectedVariant.price)}</span>
            </div>
