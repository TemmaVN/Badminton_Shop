...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!acceptTerms) { setError('Vui lòng đồng ý với Điều khoản sử dụng!'); return; }
  ...
  return (
    <div className='flex w-full h-auto text-black'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col grow max-w-150 mx-10 my-16 md:mx-20 justify-center gap-1'>
        ...
        <label className='pt-6'>Mật khẩu</label>
        ...
        <label className='pt-6'>Nhập lại mật khẩu</label>
        ...
        <div className='flex gap-2 py-3'>
          <input type="checkbox" className='size-4 accent-orange-500 cursor-pointer mt-0.5' checked={acceptTerms} onChange={(e) => setAcceptTerms(e.target.checked)} />
          <label className='text-sm'>Tôi đồng ý với Điều khoản sử dụng dịch vụ</label>
        </div>
        ...
        <div className={`flex ${isShowPic ? '' : 'justify-center'}`}>
          <div className='flex gap-2 py-3 text-sm'>
            ...
            <Link to="/login" className='text-orange-default hover:text-orange-dark'>Đăng nhập</Link>
          </div>
        </div>
      </form>
    </div>
