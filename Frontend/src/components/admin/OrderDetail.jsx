...
const STATUS_INFO = {
  ...
  9: { text: "Đang yêu cầu trả hàng/hoàn tiền", badge: "bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400" },
  10:{ text: "Đã chấp thuận trả hàng/hoàn tiền", badge: "bg-cyan-100 dark:bg-cyan-500/15 text-cyan-700 dark:text-cyan-400" },
  11:{ text: "Đã hoàn hàng/hoàn tiền", badge: "bg-rose-100 dark:bg-rose-500/15 text-rose-700 dark:text-rose-400" },
};
...
// Chuẩn hóa status (BE trả về chuỗi tiếng Việt hoặc số) -> id số
const resolveStatusId = (rawStatus) => {
  if (rawStatus === null || rawStatus === undefined) return 1;
  const parsedId = parseInt(rawStatus, 10);
  if (!isNaN(parsedId) && parsedId > 0 && parsedId <= 11 && String(parsedId) === String(rawStatus).trim()) {
    return parsedId;
  }
  ...
  if (normalized.includes("huỷ") || normalized.includes("hủy")) return CANCEL_STATUS_ID;
  if (!isNaN(parsedId) && parsedId > 0 && parsedId <= 11) return parsedId;
  return 1;
};
...
const OrderDetail = ({ order, onClose, onUpdate }) => {
  ...
  // Theo state machine: hủy được phép từ 1,2,3,5 (không phải 4=đan lưới, không phải 6,7,8+)
  const canCancel = !isCancelled && !isCompleted && [1, 2, 3, 5].includes(currentStatusId);

  const refreshDetail = async () => {
    try {
      const res = await orderApi.getAdminDetail(localOrder.orderId);
      setLocalOrder(res.data?.data || res.data);
    } catch {
      // giữ nguyên localOrder nếu refetch lỗi
    }
  };

  const handleUpdateStatus = async (newStatusId) => {
    if (newStatusId === CANCEL_STATUS_ID) {
      let cancelReason = window.prompt("Nhập lý do hủy đơn hàng:");
      ...
      try {
        ...
        alert("Hủy đơn thất bại: " + (error.response?.data?.message || error.message));
      } finally {
        setIsUpdating(false);
      }
      return;
    ...
    setIsUpdating(true);
    // updateOrderStatus của context gửi số nguyên thô theo đúng contract BE
    const result = await updateOrderStatus(localOrder.orderId, newStatusId);
    if (result.success) {
      await refreshDetail();
      if (onUpdate) onUpdate();
    } else {
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-5 mb-5">
            ...
            <div className="flex justify-between items-start w-full px-2 mt-4">
              {STATUS_TIMELINE.map((status, index) => {
                ...
                const isLineActive = nextStatus && currentStatusId >= nextStatus.id;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 p-5">
              ...
              {isCancelled && localOrder.cancelReason && (
                <div className="mt-4 pt-4 border-t border-red-100 dark:border-red-500/20 flex items-start gap-2 text-sm text-red-600 dark:text-red-400">
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                {(localOrder.orderDetails || []).map((item, idx) => (
                  <tr key={item.orderDetailId ?? idx} className="text-sm hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="p-4 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {item.isStringingService && (
                          <p className="text-xs text-fuchsia-500 dark:text-fuchsia-400">
                            Dịch vụ đan lưới{item.stringBrand ? ` · ${item.stringBrand}` : ""}{item.tensionKg ? ` · ${item.tensionKg}kg` : ""}
                          </p>
                        )}
