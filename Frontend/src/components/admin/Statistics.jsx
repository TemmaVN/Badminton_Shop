          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip contentStyle={makeTooltipStyle()} formatter={(v, n) => [`${v} triệu đ`, n === 'doanhThu' ? 'Doanh thu' : n === 'chiPhi' ? 'Chi phí' : 'Lợi nhuận']} />
                <Legend formatter={(v) => v === 'doanhThu' ? 'Doanh thu' : v === 'chiPhi' ? 'Chi phí' : 'Lợi nhuận'} />
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={compareData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="thang" {...axisProps} />
                <YAxis tickFormatter={fmtM} {...axisProps} />
                <Tooltip contentStyle={makeTooltipStyle()} formatter={(v, n) => [`${v} triệu đ`, n]} />
                <Legend />
            ...
            <tbody>
              {data.map((d, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-4 text-right text-slate-500 dark:text-slate-400">{Math.round((d.doanhThu * 1_000_000) / d.donHang).toLocaleString('vi-VN')} đ</td>
                </tr>
              ))}
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip contentStyle={makeTooltipStyle()} formatter={(v) => [`${v}%`]} />
              </PieChart>
            </ResponsiveContainer>
          </div>
            ...
            <tbody>
              {categoryData.map((cat, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-4 text-right font-semibold text-orange-500">{(cat.doanhThu * 1_000_000).toLocaleString('vi-VN')} đ</td>
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={brandData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 0 }}>
              ...
              <Tooltip contentStyle={makeTooltipStyle()} formatter={(v) => [`${v} triệu đ`, 'Doanh thu']} />
        ...
        <div className="p-6 space-y-4">
          {brandData.map((brand, i) => (
            <div key={i} className="flex items-center gap-4">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-xs font-black shrink-0 shadow" style={{ backgroundColor: BRAND_COLORS[i] }}>
                {brand.ten[0]}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-700" style={{ width: `${(brand.doanhThu / maxDT) * 100}%`, backgroundColor: BRAND_COLORS[i] }} />
                </div>
              </div>
              <span className="text-sm font-bold text-slate-800 dark:text-white shrink-0 w-14 text-right">{brand.doanhThu}M</span>
            </div>
          ))}
        </div>
      </Card>
      ...
      <div className="p-6 border-b border-slate-200/50 dark:border-slate-700/50">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Top {filtered.length} sản phẩm</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="text-sm border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-orange-400">
              <option value="all">Tất cả danh mục</option>
              {cats.slice(1).map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
            <select value={filterTrend} onChange={(e) => setFilterTrend(e.target.value)} className="text-sm border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-orange-400">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="p-12 text-center text-slate-400 dark:text-slate-600">Không có dữ liệu phù hợp</td>
              </tr>
            ) : (
              filtered.map((p, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="p-4 text-center">
                    <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${p.rank <= 3 ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/30' : 'bg-slate-100 text-slate-500 dark:bg-slate-800'}`}>{p.rank}</span>
                  </td>
                  <td className="p-4 font-medium text-slate-800 dark:text-white">{p.ten}</td>
                  <td className="p-4 hidden md:table-cell"><span className="px-2 py-0.5 rounded-full text-xs bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">{p.danhMuc}</span></td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      {p.trend === 'up' ? <TrendingUp className="w-3.5 h-3.5 text-emerald-500" /> : <TrendingDown className="w-3.5 h-3.5 text-rose-500" />}
                      <span className={`text-xs font-semibold ${p.trend === 'up' ? 'text-emerald-500' : 'text-rose-500'}`}>{p.change}</span>
                    </div>
                  </td>
                </tr>
              ))
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={orderTrendData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="thang" {...axisProps} />
                <YAxis {...axisProps} />
                <Tooltip contentStyle={makeTooltipStyle()} />
                <Legend />
        ...
        <Card className="p-6">
          ...
          <div className="space-y-2 mt-2">
            {orderStatusData.map((item, i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: item.color }} /><span className="text-xs text-slate-600 dark:text-slate-400">{item.name}</span></div>
    ...
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <select value={period} onChange={(e) => setPeriod(e.target.value)} className="text-sm border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-orange-400">
          ...
          <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 text-sm font-medium transition-colors">
            <Download className="w-4 h-4" /><span className="hidden sm:inline">Xuất báo cáo</span>
          </button>
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {SUMMARY.map((s, i) => (
          <div key={i} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-5 border border-slate-200/50 dark:border-slate-700/50">
            ...
            <div className="flex items-center gap-1 mt-2">
              {s.trend === 'up' ? <TrendingUp className="w-3.5 h-3.5 text-emerald-500" /> : <TrendingDown className="w-3.5 h-3.5 text-rose-500" />}
      ...
      <div className="flex overflow-x-auto gap-2 pb-1">
        {TABS.map((tab) => {
          ...
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${active ? 'bg-orange-default text-white shadow-lg shadow-orange-default/25' : 'bg-white/80 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
