            ...
            <tbody>
              {catData.map((cat, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-4 text-right font-semibold text-orange-500">
                    {(cat.doanhThu ?? 0).toLocaleString('vi-VN')} đ
                  </td>
                  <td className="p-4 text-right text-slate-600 dark:text-slate-400">{(cat.donHang ?? 0).toLocaleString()}</td>
                  <td className="p-4 text-right text-slate-600 dark:text-slate-400">{(cat.daBan ?? 0).toLocaleString()} sp</td>
            ...
            <div key={i} className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-xs text-slate-400">{(brand.donHang ?? 0).toLocaleString()} đơn</span>
                </div>
                <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((p, i) => (
                <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="p-4 text-right text-slate-700 dark:text-slate-300">{(p.daBan ?? 0).toLocaleString()}</td>
...
function OrdersTab() {
  ...
  const totalOrders = orderStatus.reduce((s, r) => s + (r.totalOrders ?? 0), 0);

  const completedOrders = orderStatus
    .filter(r => [6, 7].includes(r.statusId))
    .reduce((s, r) => s + (r.totalOrders ?? 0), 0);
  ...
  const donutData = STATUS_GROUPS.map(g => ({
    name:  g.label,
    value: orderStatus.filter(r => g.ids.includes(r.statusId)).reduce((s, r) => s + (r.totalOrders ?? 0), 0),
    color: g.color,
  })).filter(d => d.value > 0);
...
export default function Statistics() {
  ...
  const { overview, fetchOverview, fetchRevenueByMonth } = useStatistic();

  // Tải dữ liệu tổng quan & doanh thu theo tháng (năm 2025) từ backend
  useEffect(() => {
    fetchOverview();
    fetchRevenueByMonth();
  }, [fetchOverview, fetchRevenueByMonth]);
  ...
  const summaryCards = useMemo(() => [
    ...
    {
      title: "Tổng đơn hàng",
      value: ovData ? (ovData.totalOrders ?? 0).toLocaleString() : '—',
      color: "text-blue-500",
    ...
    {
      title: "Khách hàng",
      value: ovData ? (ovData.totalCustomers ?? 0).toLocaleString() : '—',
