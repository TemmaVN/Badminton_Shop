...
const STATUS = {
    ...
    'Reserved': { label: 'Đã đặt',   cls: 'bg-orange-50 dark:bg-orange-500/15 text-orange-600 dark:text-orange-400 border-orange-100 dark:border-orange-500/30',      dot: 'bg-orange-500'  },
};
...
// metaKey khớp với MetaData (Backend_ThucTap): weightClasses, gripSizes, balancePoints, stiffness, maxTensions
const FORM_FIELDS = [
    { label: 'Hạng trọng lượng', metaKey: 'weightClasses',  formKey: 'weightClass'  },
...
const AdminProductDetail = () => {
    ...
    const ensureSerials = async (detailId) => {
        ...
        try {
            ...
            setSerialsMap((prev) => ({ ...prev, [detailId]: res.data?.data ?? res.data ?? null }));
        } catch {
    ...
    const handleAddSerialModal = async () => {
        ...
        const statusStr = STATUS_STRING[statusInt] ?? 'InStock';
        const importDate = addSerialModal.importDate || new Date().toISOString().split('T')[0];
        try {
            await productApi.addSerial(detailId, { serialNumber: sn, status: statusStr, importDate });
            const newEntry = { serialNumber: sn, status: statusStr, importDate: new Date(importDate).toISOString() };
    ...
    const handleEditVariant = async () => {
        ...
        try {
            ...
            // Làm mới serial của biến thể vừa sửa (res.data?.data, không phải res.data.data.data)
            const res = await productApi.getSerials(editingVariant.detailId, { page: 1, pageSize: 200 });
            setSerialsMap((prev) => ({ ...prev, [editingVariant.detailId]: res.data?.data ?? res.data ?? null }));
    ...
    const handleAddSerial = async (detailId) => {
        ...
        try {
            await productApi.addSerial(detailId, { serialNumber: sn, status: 'InStock' });
            const newEntry = { serialNumber: sn, status: 'InStock', importDate: new Date().toISOString() };
            setSerialsMap((prev) => {
                const cur = prev[detailId];
                                            ...
                                            <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-700 rounded-xl p-4">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {serials.serials?.map((sn, idx) => {
                                                        const s = STATUS[sn.status] ?? STATUS['InStock'];
                                                        return (
                                                            <div
                                                                key={idx}
