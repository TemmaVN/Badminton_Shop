...
import { returnApi, productApi } from '../../api';

const PAGE_SIZE = 10;

const STATUS_MAP = {
  'Chờ xử lý':     { bg: 'bg-amber-50 dark:bg-amber-900/20',   text: 'text-amber-700 dark:text-amber-300',   border: 'border-amber-200 dark:border-amber-500/30',   dot: 'bg-amber-400' },
...
const DetailModal = ({ request, onClose, onUpdate }) => {
  ...
  // Backend_ThucTap: minh chứng giao hàng là JSON { imageUrl, note }, không phải upload file multipart.
  // Cho phép admin nhập trực tiếp URL ảnh, hoặc chọn file để tải lên trước rồi lấy URL.
  ...
  const isPending  = request.status === 'Chờ xử lý';
  const isApproved = request.status === 'Đã chấp thuận';

  // Tuỳ chọn: tải file ảnh lên trước để lấy URL (dùng chung endpoint upload ảnh sản phẩm)
  const handleProofFileUpload = async (e) => {
    ...
    try {
      ...
      else setError('Không lấy được URL ảnh sau khi tải lên.');
    } catch (err) {
      setError(err.response?.data?.message ?? err.response?.data?.Message ?? 'Tải ảnh thất bại.');
    } finally {
  ...
  const submit = async () => {
    ...
    try {
      if (action === 'approve') {
        ...
        if (!proofUrl.trim()) { setError('Vui lòng nhập URL ảnh minh chứng.'); setSubmitting(false); return; }
        await returnApi.addDeliveryProof(request.orderId, proofUrl.trim(), proofNote.trim() || null);
      }
      onUpdate();
      onClose();
    } catch (e) {
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          ...
          {action === 'proof' && (
            <div className="border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
              ...
              {/* Backend_ThucTap: nhận URL ảnh (JSON), không phải file multipart. */}
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {proofUrl && (
                    <img src={proofUrl} alt="preview" className="w-10 h-10 object-cover rounded-lg" onError={(e) => { e.target.style.display = 'none'; }} />
                  )}
                </div>
              </div>
              <div>
              ...
              {error && <p className="text-xs text-red-500 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-500/30 rounded-lg px-3 py-2">{error}</p>}
              <div className="flex gap-2">
                <button onClick={() => { setAction(null); setError(null); setProofUrl(''); setProofNote(''); }}
                  className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Huỷ
                </button>
                <button onClick={submit} disabled={submitting}
