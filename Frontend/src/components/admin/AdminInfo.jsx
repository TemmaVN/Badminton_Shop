...
const AdminInfo = () => {
   ...
   const [adminData, setAdminData] = useState(getAdminFromStorage());
   ...
   // const { changePassword, UpdateProfile} = useUser();
    ...
    const handleUpdateProfile = async (e) => {
        ...
        // Cập nhật dữ liệu admin trong state và localStorage
        const updatedAdmin = { ...adminData, ...editForm };
        setAdminData(updatedAdmin);
        localStorage.setItem('user', JSON.stringify(updatedAdmin));
        setMessage({ type: 'success', text: 'Cập nhật hồ sơ thành công!' });
        // Chuyển về tab profile sau 1 giây
        setTimeout(() => {
     ...
     const handleChangePwd = async (e) => {
        ...
        // Giả sử mật khẩu cũ đúng là "admin123" (có thể thay đổi)
        const isValidOld = (pwdForm.oldPassword === 'admin123');
        if (!isValidOld) {
            return setMessage({ type: 'error', text: 'Mật khẩu hiện tại không đúng!' });
        }
        setActionLoading(true);
        await new Promise(resolve => setTimeout(resolve, 800));
