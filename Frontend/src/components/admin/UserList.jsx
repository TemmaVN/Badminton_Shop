...
const MOCK_USERS = [
  {
    ...
    city: "Hồ Chí Minh",
    district: "Quận 1",
    detailedAddress: "123 Lê Lợi, P. Bến Thành",
    roles: ["Customer"],
  },
  {
    userId: 2,
  ...
  },
  {
    ...
    city: "Hồ Chí Minh",
    district: "Quận 3",
    detailedAddress: "321 Nguyễn Đình Chiểu",
    roles: ["Customer"],
  },
  {
    userId: 5,
...
const UserList = () => {
  ...
  const [createError,     setCreateError]     = useState('');
   ...
   // Hàm lọc danh sách khách hàng theo keyword (tên, email, phone)
  const filterUsers = (users, kw) => {
    if (!kw) return users;
    const lowerKw = kw.toLowerCase();
    return users.filter(u =>
      (u.fullName && u.fullName.toLowerCase().includes(lowerKw)) ||
      (u.email && u.email.toLowerCase().includes(lowerKw)) ||
      (u.phoneNumber && u.phoneNumber.includes(lowerKw))
    );
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          const filtered = filterUsers(MOCK_USERS, keyword);
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 300));
      // Tạo user mới (không lưu password thật, nhưng vẫn giữ trường password trong form)
      const newUser = {
        userId: Date.now(), // dùng timestamp làm id giả
      ...
      const newList = [newUser, ...MOCK_USERS];
      // Cập nhật lại state với dữ liệu mới (áp dụng filter hiện tại)
      const filteredNew = filterUsers(newList, keyword);
      setCustomers(filteredNew);
      setShowCreateModal(false);
      setCreateForm(EMPTY_USER);
      // Không tự động reset keyword, nhưng hiển thị người dùng mới trong danh sách
    } catch (err) {
      setCreateError('Không thể tạo tài khoản, vui lòng thử lại.');
    } finally {
