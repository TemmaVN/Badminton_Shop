      ...
      <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-sm text-slate-600 dark:text-slate-300">
        <span>
          ...
          <strong className="text-slate-800 dark:text-white">
            {v.isPercent ? `${v.discountValue}%` : `${(v.discountValue ?? 0).toLocaleString()}₫`}
          </strong>
        ...
        <span>
          Đơn tối thiểu:{" "}
          <strong className="text-slate-800 dark:text-white">{(v.minOrderValue ?? 0).toLocaleString()}₫</strong>
        </span>
        <span>
          Hết hạn:{" "}
          <strong className="text-slate-800 dark:text-white">
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  {loadingList ? "Đang tải..." : `${vouchers.length} voucher trong hệ thống`}
                </p>
                <button
